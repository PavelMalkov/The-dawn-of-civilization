...
public class Clouds : MonoBehaviour
{
    ...
    void SpawnClouds(int Number, float X)
    {
        ...
        // Создание объекта
        Instantiate(Cloud[Number], SpawnCloudsPosition, Quaternion.identity).transform.SetParent(this.transform,false);
    }
    ...
    // Генератор одного случайного облака с левой стороны
    void SpawnClouds()
    {
        // Положение объекта
        SpawnCloudsPosition = new Vector2(RandomXLeft, Random.Range(RandomYTop, RandomYDown));
        // Создание объекта
        Instantiate(Cloud[Random.Range(0, Cloud.Length)], SpawnCloudsPosition, Quaternion.identity).transform.SetParent(this.transform, false);
    }
